...
// - PARCEL_DETAILS_PRIM_CAPACITY and PARCEL_DETAILS_PRIM_USED report the
//   same-owner simulator-wide capacity/usage values expected by SL scripts.
//
// Setup:
...
string parcel_capacity_row(vector pos)
{
    list details = llGetParcelDetails(pos, [
        PARCEL_DETAILS_PRIM_CAPACITY,
        PARCEL_DETAILS_PRIM_USED
    ]);

    integer capacity = llList2Integer(details, 0);
    integer used = llList2Integer(details, 1);
    integer free = capacity - used;

    return "DETAILS: capacity=" + (string)capacity + " used=" + (string)used + " free=" + (string)free;
}

string owner_rows(vector pos)
{
    list owners = llGetParcelPrimOwners(pos);
    integer length = llGetListLength(owners);
    integer i;
    string out = "";

    for (i = 0; i + 1 < length && i < 12; i += 2)
    {
        out += "\nOWNER " + (string)((i / 2) + 1) + ": " +
            llList2String(owners, i) + " = " +
            (string)llList2Integer(owners, i + 1);
    }

    if (length == 0)
        out = "\nOWNER: none";
    else if (length > 12)
        out += "\nOWNER: +" + (string)((length - 12) / 2) + " more";
...
{
    say_to(agent,
        "HERE audits the parcel under this object. SCAN audits the nearest detected avatar/object position. REPORT repeats the last audit. sameOwnerSim is llGetParcelPrimCount(pos, category, TRUE), summed across parcels with the same land owner. DETAILS uses llGetParcelDetails capacity/used.");
}
